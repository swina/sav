...
function setPlan(){
	...
	var myXML = "_planXML.cfm?dateFrom=" + getValore("dateFrom") + "&dateTo=" + getValore("dateTo") + "&id_agente=" + id_agente[0] + "&id_gruppo_agenti=" + id_gruppo_agenti[0] + "&id_processo=" + id_processo[0];
	
	mygrid.clearAndLoad(myXML);
}

function doDettaglioCliente(rId,cIn){
	var id_cliente = mygrid.getUserData(rId,"id_cliente");
	obj("detail").src = "_planCliente.cfm?id_cliente=" + id_cliente;
	hideObj("planXMLView");
	showObj("detailCliente");
}
