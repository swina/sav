...
//------------------------------------------------------------------------------
	...
	function doSelectFirstRow(){
		//seleziona la prima riga quando la grid � renderizzata
		if ( currentRow == 0 ){
			mygrid.selectRow(0);
		} else {
	...
	function doOnRowSelect(id,cella){
		...
		if ( debug == 0 ){
			...
			GB_showCenter("Anagrafica", "../../clienti/clientiFormIstat.cfm?idc=" + idc, 500, 450,reloadData);
		} else {
	...
	//importa contatti da segnalazioni mail
	function importaClienti(){
		window.open("_clientiImportaMail.cfm");
		vediNuoviContatti();
		//ColdFusion.Ajax.submitForm("deleteForm","_clientiImportaMail.cfm",callbackClientiImportaMail,errorHandler);
	}
	
	function callbackClientiImportaMail(result){
		alert(result);
		vediNuoviContatti()
	}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
	...
	function doGetData(){
		...
		var agente_assegnato = comboGet("id_agente_assegnato");
		var dFrom = getValore("dateFrom");
		var dTo = getValore("dateTo");
		var fornitore = getValore("ac_segnalatore_assegnato");
		var fornitore_search = getValore("fornitore_search");
		setValore("currentFilter","startpage=" + page + "&pagesize=" + pagesize + "&search=" + search + "&id_agente=" + assegnati[0] + "&id_gruppo=" + tipologia[0] + "&pv=" + pv[0] + "&import=" + attivo + "&id_agente_assegnato=" + agente_assegnato[0] + "&dFrom=" + dFrom + "&dTo=" + dTo + "&fornitore=" + fornitore );
		mygrid.clearAndLoad("_clientiFilterXML.cfm?startpage=" + page + "&pagesize=" + pagesize + "&search=" + search + "&id_agente=" + assegnati[0] + "&id_gruppo=" + tipologia[0] + "&pv=" + pv[0] + "&import=" + attivo + "&id_agente_assegnato=" + agente_assegnato[0] + "&dFrom=" + dFrom + "&dTo=" + dTo + "&fornitore=" + fornitore + "&fornitore_search=" + fornitore_search);
		//alert(attivo);
	}
	...
	function vediArchivio(){
		setValore("bl_attivo",-2);
		doGetData();
	}
...
//--------------------------------------------------------------------------
	...
	function doAssegnaMultiple(){
		//verifica che non sia gi� stato cliccato 
	...
	//importa i contatti dalle mail arrivate	
	function importaContatti(){
		obj("working").src = "../import/_importMail.cfm";
	}
