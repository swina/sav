...
//----------------------------------------------------------------
	...
	function statusCliente(rId , cId){
		...
		//è stato compilato un modulo per l'azione selezionata
	...
	//utenti può essere assegnata un'azione specifica. In questo caso l'azione sarà visibile solo 
	...
	function getDataStatus(){
		...
		//verifico se l'azione è collegata ad un modulo 	
		if ( status_modulo_uuid != "" && status_modulo != "No" ){
			//esiste già un modulo compilato
	...
	//se il valore nella colonna int_tipo della tabella processi è = 2 
	//blocca il campo data. L'utente non può immettere una data diversa
	//da quella odierna
	...
	//che la data sia inferiore di int_timer_limit/24 (giorni) al processo da cui è vincolato
...
ColdFusion.Ajax.submitForm("processoFrm","_statusDeleteProcesso.cfm",callback,errorHandler);
	...
	function getFileNameUploaded(obj){
	  	//verifica se caricati più file contemporaneamente
...
ColdFusion.Ajax.submitForm("processoFrm","_statusDocsMove.cfm",callbackDocsMove,errorHandler);
	...
	function checkModulo(){
		...
		if ( myModuli[indice] != "No" ){
			...
			if ( getValore("modulo_uuid") == "" ){
				GB_showCenter(myModuli[indice], "../../status/_statusModulo.cfm?text=" + myModuli[indice] + "&id_cliente=" + idcliente, 350, 400, callBackModulo);
				//setValore("modulo_flag","1");
			} else {
				setValore("modulo_flag","1");
				GB_showCenter(myModuli[indice], "../../status/_statusModulo.cfm?text=" + myModuli[indice] + "&uuid=" + getValore("modulo_uuid"), 350, 400, callBackModulo);
	...
	//funzione per visualizzare il modulo già compilato
