	...
	*/
	...
	this._xmlParser = function() {
		...
		/*
			...
			---
			buttonSelect nested item: enabled, disabled, action, selected, img, text, temText(nested)
			
		*/
		var t = ["id", "type", "hidden", "title", "text", "enabled", "img", "imgdis", "action", "openAll", "renderSelect", "width", "value", "selected", "length", "textMin", "textMax", "toolTip", "valueMin", "valueMax", "valueNow"];
...
*   @type: public
*/
dhtmlXToolbarObject.prototype.addButtonSelect = function(id, pos, text, opts, imgEnabled, imgDisabled, renderSelect, openAll) { 
	var items = [];
	...
	this._addItem({id:id, type:"buttonSelect", text:text, img:imgEnabled, imgdis:imgDisabled, renderSelect:renderSelect, openAll:openAll, items:items}, pos);
}
//#}
...
dhtmlXToolbarObject.prototype._buttonSelectObject = function(that, id, data) {
	...
	this._buttonButtonSelectObject = function(id, data, pos) {
		...
		*/
		...
		this.obj.tr.onmouseover = function() {
		...
		for (var a in this._listOptions) {
			...
			if (item.td_a != null && item.td_b != null && item.tr.en) {
				if (a == id) {
					...
					item.className = "tr_btn"+(this.renderSelect?" tr_btn_selected":"");
				} else {
...
// user data start
/*
dhtmlXToolbarObject.prototype._parseXMLUserData = function(id, itemData) {
	var data = itemData.getElementsByTagName("userdata");
	for (var q=0; q<data.length; q++) try {this.setUserData(id, data[q].getAttribute("name"), data[q].firstChild.nodeValue);} catch(e){}
};
*/
