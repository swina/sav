...
dhtmlXWindows.prototype._dhx_Engine = function() {
	...
	this._engineCheckHeaderMouseDown = function(win, ev) {
		...
		var hh = (win._noHeader==true?win._hdrSize:this._engineSkinParams[this.skin]["hh"]);
		if (y <= hh && (obj == win.childNodes[0] || obj == win.childNodes[0].childNodes[(_isIE?1:0)] || obj == win.childNodes[0].childNodes[(_isIE?3:2)] || obj == win.childNodes[0].childNodes[(_isIE?4:3)])) return true;
		return false;
	}
	...
	this._engineGetWindowContent = function(win) {
		...
		/*
		var contObj = null;
		var contPoly = win.childNodes[0].childNodes[(_isIE?5:4)];
	...
	this._engineSwitchWindowHeader = function(win, state) {
		...
		// title
		win.childNodes[0].childNodes[(_isIE?3:2)].style.display = (win._noHeader?"none":"");
		// buttons
		win.childNodes[0].childNodes[(_isIE?4:3)].style.display = (win._noHeader?"none":"");
		//
		var sk = this._engineSkinParams[this.skin];
		if (win._noHeader) {
			win.dhxContGlobal.obj._offsetHeightSaved = win.dhxContGlobal.obj._offsetHeight;
	...
	this._engineRedrawWindowTitle = function(win) {
		if (win._noHeader!==true) {
			var p2 = win.childNodes[0].childNodes[(_isIE?1:0)].offsetWidth; // icon
			var p3 = win.childNodes[0].childNodes[(_isIE?4:3)].offsetWidth; // buttons
