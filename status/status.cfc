<cfcomponent>
	...
	<cffunction name="getProcessi" access="remote" returntype="query">
		<cfquery name="qryP" datasource="#THIS.dsn#">
	...
	<cffunction name="addProcesso" access="remote" returntype="any">
		...
		<cfset data_status = CreateDateTime(ListGetAt(arguments.dt_status,3,"/"),ListGetAt(arguments.dt_status,2,"/"),ListGetAt(arguments.dt_status,1,"/"),Int(ListGetAt(arguments.ac_ora,1,":")),Int(ListGetAt(arguments.ac_ora,2,":")),0)>
			...
			<cfif arguments.id_processo EQ 6>
				<cfquery name="checkPresentazione" datasource="#THIS.dsn#">
					SELECT id_status , dt_status 
					WHERE id_cliente = #arguments.id_cliente# 
					AND id_processo = 4 
					ORDER BY dt_status DESC
				</cfquery>
				<cfset data_nuova = CreateDateTime(year(checkPresentazione.dt_status),month(checkPresentazione.dt_status),day(checkPresentazione.dt_status),hour(checkPresentazione.dt_status),minute(checkPresentazione.dt_status),0)>
			</cfif>
			
			<cfif arguments.id_processo EQ 14>
				<cfset data_nuova = DateAdd("d",7,data_status)>
			</cfif>
	...
	<cffunction name="deleteProcesso" access="remote" returntype="any">
		<cfargument name="id_status" required="Yes" type="numeric">
		<cfquery name="dati" datasource="#THIS.dsn#">
...
<!--- 		<cfif docs NEQ "">
		...
		<cfif dati.id_processo EQ 9>
			<cfset id_cliente = dati.id_cliente>
			<cfquery name="updateRichiesta" datasource="#THIS.dsn#">
			UPDATE tbl_status
			SET	bl_evasa = 0
			WHERE id_cliente = #id_cliente# AND id_processo = 6
			</cfquery>
		</cfif>
		<cfquery name="qry" datasource="#THIS.dsn#">
	...
	<cffunction name="moveDocs" access="remote" returntype="any">
		...
		<cfloop index="i" list="#arguments.lista_docs#">
			<cfif fileexists("#THIS.docs_folder#\#i#")>
				<cffile action="MOVE" source="#THIS.docs_folder#\#i#" destination="#THIS.docs_folder#\#arguments.id_cliente#\#i#">
			</cfif>
		</cfloop>
		...
		<cfquery name="qry" datasource="#THIS.dsn#">
			UPDATE tbl_status
			SET ac_docs = '#arguments.lista_docs#'
